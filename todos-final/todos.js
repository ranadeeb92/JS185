...
catchError(
  async(req, res) => {
    let todoListId = req.params.todoListId;
    let store = res.locals.store;
    let AllMarkedDone = await store.completeAllTodos(+todoListId);
...
app.post("/lists/:todoListId/todos",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      todoList.todos = await store.storedTodos(todoList);
      ...
      let created = await store.createTodo(todoTitle, +todoListId);
      if(!created) throw new Error('Failed to create a new todo');
    ...
    async(req, res) => {
      ...
      try{
        ...
        if (!errors.isEmpty()) {
          ...
          let edited = await store.editTodoList(todoListTitle, +todoListId);
          if(!edited) throw new Error("Faild to edit todolist title");
